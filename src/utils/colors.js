/**
 * HEX codes of colours throughout the App
 */
export const COLOR = {
    WHITE: '#fff',
    BLACK: '#000',
    RED: 'red',
    HOME_VALUATION : '#e8a7db',
    GRAY: '#B8B6B6',
    LIGHTER_GRAY: '#E2E2E2',
    REDUX_TEXTINPUT_TEXT: '#666666',
    DARKER_GRAY: '#7C7C7C',
    REDUX_FORM_INPUT_CONTAINER: '#0000004D',
    PRIMARY: '#DD2371',
    LESS_OPACITY_PRIMARY: 'rgba(221, 35, 113, 0.3)',
    BLUE: '#0027FF',
    BORDER_BLACK: '#0000001A',
    VOILET: '#09245E',
    BLACKISH_GRAY: '#908F8F',
    LIGHT_VOILET: '#22319B80',
    OVERPAYMENT_CARD: '#22319B',
    DARK_VOILET: '#3742A8',
    CONTAINER_GREY : '#f6f6f6',
    DISABLED_COLOR: '#CED3DF',
    DARK_BLUE: '#22319B',
    DARK_YELLOW: '#FFB400',
    LIGHT_YELLOW: '#FFB4004D',
    LIGHT_GREEN: '#00D77E',
    LIGHT_GRAY: '#E6E9EF',
    DOLLAR_COLOR: '#09245E4D',
    GREEN: '#00ff00',
    PARROT_GREEN: '#1FCC33',
    DARK_GRAY: '#707070',
    SHADOW_BORDER: '#ddd',
    SHADOW_COLOR: '#000',
    LIGHT_BORDER: 'rgba(9, 36, 94, 0.1)',
    LIGHT_TEXT_GREEN: 'rgba(0, 193, 164, 1)',
    LIGHT_TEXT_GREEN_MILD_OPACITY: 'rgba(0, 193, 164, 0.2)',
    LIGHT_TEXT_GREEN_LEAST_OPACITY: 'rgba(0, 193, 164, 0.1)',
    SLIDER_COLOR: '#00C1A4',
    STEEL_GRAY: '#D3D6EB',
    BRICK_RED: '#FF7474',
    INACTIVE_TAB: '#D8D8D8',
    DARKEST_YELLOW: '#FF9D00',
    LIGHTEST_YELLOW: '#FFE9B3',
    SLIGHT_YELLOW: '#FFF0CC',
    SLIGHT_BLUE: '#22319B66',
    BLACK_SHADOW_COLOR: '#0000000F',
    PLACEHOLDER: 'rgba(9, 36, 94, 0.5)',
    SHADED_GREEN: '#A0FFD8',
    TRANSPARENT_BLACK: 'rgba(0,0,0,0.5)',
    COUNTER_BG_COLOR: '#4E5AAF',
    LIGHT_OPACITY_BLACK: 'rgba(0,0,0,0.5)',
    LIGHTER_VOILET: '#4D56B1',
    BLACK_OPACITY_TEN: 'rgba(0, 0, 0, 0.1)',
    MEDUIM_OPACITY_BLACK: 'rgba(0, 0, 0, 0.3)',
    HALF_VOILET: 'rgba(9, 36, 94, 0.5)',
    PARTIAL_BLACK: 'rgba(9, 36, 94, 0.3)',
    ALICE_BLUE: '#F7F8FA',
    SMOKE_WHITE: '#F5F5F5',
    LIGHT_BORDER_LEAST_OPACITY: 'rgba(9, 36, 94, 0.02)',
    MILD_BACKGROUND_BLACK: 'rgba(9, 36, 94, 0.05)',
    SALMON_RED: '#FF7474',
    AMBER: '#FFB400',
    GHOST_WHITE: '#F2F3F9',
    TRACK_BLUE: 'rgba(34, 49, 155, 0.5)',
    GRADIENT_PRIMARY: 'rgba(221, 35, 113, 0.1)',
    HUE_BLUE: '#09245E80',
    CARIBBEAN_GREEN: '#00C1A4',
    PRIMARY_THIRD_PART: 'rgba(221, 35, 113, 0.3)',
    CARIBBEAN_GREEN_ONE_THIRD: 'rgba(30, 193, 164, 0.3)',
    VOILET_ONE_THIRD: 'rgba(9, 36, 94, 0.3)',
    WIAKAWA_GRAY: '#5E6F98',
    HAWKES_BLUE: '#D2DAED',
    SHADOW_COLOR_LATEST: '#00000029',
    LIGHT_BORDER_MARGIN: 'rgba(0, 0, 0, 0.1)',
    SHADOW_BLUE: '#7E8BAB',
    SLIGHT_BLUE_WHITEL: '#F2F3F8',
    TOGGLE_DISABLED_GRAY: '#78788029',
    TOGGLE_ENABLED_GREEN: '#34C759',
    GRAYISH: '#D3D3D3',
    CODE_INPUT_BG_COLOR: '#00000030',
    BEIGE_GRAY: '#F4F4F4',
    CLEAR_ALL_COLOR: 'rgba(126, 140, 169, 0.2)',
    NOTIFICATION_ERROR: '#FE7475',
    NOTIFICATION_BLOG: '#FFB436',
    NOTIFICATION_ACTION: '#17C1A4',
  };